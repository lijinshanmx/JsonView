...
public class JsonProcessor {
    ...
    public JsonProcessor(RecyclerView.Adapter jsonAdapter) {
        if (jsonAdapter == null) return;
        this.jsonAdapter = jsonAdapter;
        jsonAdapter.registerAdapterDataObserver(new RecyclerView.AdapterDataObserver() {
            @Override
    ...
    private void expandOrCollapseJsonItemInternal(boolean collapse, JsonItemBean jsonItemBean) {
        for (JsonItemBean itemBean : jsonItemBeans) {
            if (!collapse) {
                if (itemBean.parent == jsonItemBean) {
                ...
                if (itemBean.parent == jsonItemBean) {
                    ...
                    if (itemBean.isNode) { // node节点，折叠且显示
                        ...
                        expandOrCollapseJsonItemInternal(true, itemBean);
                    }
                }
            }
        }
    ...
    public void expandOrCollapseJsonItems(boolean collapse, JsonItemBean jsonItemBean) {
        for (JsonItemBean itemBean : jsonItemBeans) {
            if (itemBean.parent == jsonItemBean) {
    ...
    public JsonItemBean getViewJsonItem(int position) {
        return viewJsonItemBeans.get(position);
    }
}
